...
function getAdminClaimCacheKey(user) {
  return ADMIN_CLAIM_CACHE_PREFIX + (user?.uid || "anonymous");
}

function readCachedAdminClaim(user) {
  try {
    return localStorage.getItem(getAdminClaimCacheKey(user)) === "1";
  } catch (error) {
    console.warn("Admin yetki önbelleği okunamadı:", error);
    return false;
  }
}

function writeCachedAdminClaim(user, isAdmin) {
  try {
    localStorage.setItem(getAdminClaimCacheKey(user), isAdmin ? "1" : "0");
  } catch (error) {
    console.warn("Admin yetki önbelleği yazılamadı:", error);
  }
}
...
async function routeAfterLogin(user, options = {}) {
  ...
  try {
    ...
    const cachedAdmin = readCachedAdminClaim(user);
    const claims = await getUserClaims(user, {
      timeoutMs: cachedAdmin ? 1400 : 800,
      fallbackClaims: cachedAdmin ? { adminPanel: true } : {},
      suppressErrors: true
    });
    const isAdmin = claims.adminPanel === true;
    writeCachedAdminClaim(user, isAdmin);
    ...
    console.warn("Hızlı yönlendirme sırasında yetki kontrolü tamamlanamadı:", error);
