...
const ROUTING_WATCHDOG_MS = 7000;
...
function revealLoginScreen(force = false) {
  if (!force && (bootResolved || isRouting)) return;
...
async function routeAfterLogin(user, options = {}) {
  ...
  try {
    ...
    setButtonsDisabled(false);
    clearAppStartupSplash();
    revealLoginScreen(true);
    throw error;
  } finally {
    window.clearTimeout(routingWatchdogTimer);
  }
}
...
function init() {
  ...
  bootFallbackTimer = window.setTimeout(() => {
    setStatus("Oturum kontrolü beklenenden uzun sürdü. Giriş ekranı açıldı.");
    revealLoginScreen(true);
