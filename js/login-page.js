...
const STARTUP_SPLASH_FADE_OUT_MS = 330;
...
function markStartupSplashImageReady() {
  const targets = [document.documentElement, document.body].filter(Boolean);
  targets.forEach((target) => target.classList.add("routee-splash-ready"));

  if (elements.startupSplashImage) {
    elements.startupSplashImage.classList.add("is-loaded");
  }
}

function clearStartupSplashLeavingState() {
  const targets = [document.documentElement, document.body].filter(Boolean);
  targets.forEach((target) => target.classList.remove("routee-splash-leaving"));
  elements.startupSplash?.classList.remove("is-hiding");
}
...
function showStartupSplash(title = "Rota", message = "", options = {}) {
  ...
  clearStartupSplashLeavingState();
  const mode = options.mode === "message" ? "message" : "image";
  if (elements.startupSplashImage) {
    const src = getPreferredSplashImageUrl();
    if (elements.startupSplashImage.complete && elements.startupSplashImage.naturalWidth > 0) {
      markStartupSplashImageReady();
    }

    if (elements.startupSplashImage.getAttribute("src") !== src) {
      elements.startupSplashImage.classList.remove("is-loaded");
      document.documentElement.classList.remove("routee-splash-ready");
      document.body?.classList.remove("routee-splash-ready");
...
function hideStartupSplash() {
  ...
  const targets = [document.documentElement, document.body].filter(Boolean);
  const shouldAnimate = isMobileStartupMode() && elements.startupSplash.classList.contains("is-visible");

  elements.startupSplash.setAttribute("aria-hidden", "true");
  elements.startupSplash.dataset.mode = "image";

  const removeSplashClasses = () => {
    elements.startupSplash.classList.remove("is-visible", "is-hiding");
  ...
  targets.forEach((target) => target.classList.add("routee-splash-leaving"));
  window.setTimeout(removeSplashClasses, STARTUP_SPLASH_FADE_OUT_MS);
}
...
function init() {
  if (elements.startupSplashImage) {
    elements.startupSplashImage.addEventListener("load", markStartupSplashImageReady);
    if (elements.startupSplashImage.complete && elements.startupSplashImage.naturalWidth > 0) {
      markStartupSplashImageReady();
    }
  }
