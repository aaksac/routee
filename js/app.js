...
function syncSearchClearButton() {
  if (!elements.placeSearch || !elements.btnClearSearch) return;
  const hasValue = elements.placeSearch.value.trim().length > 0;
  elements.btnClearSearch.classList.toggle("hidden", !hasValue);
}
...
function bindEvents() {
  ...
  elements.placeSearch?.addEventListener("input", syncSearchClearButton);
  elements.btnClearSearch?.addEventListener("click", () => {
    elements.placeSearch.value = "";
    elements.placeSearch.focus();
    elements.btnClearSearch.classList.add("hidden");
    elements.placeSearch.dispatchEvent(new Event("input", { bubbles: true }));
  });
