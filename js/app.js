...
const ACCESS_CACHE_PREFIX = "routeeAccessModel:";
const ACCESS_CACHE_MAX_AGE_MS = 30 * 24 * 60 * 60 * 1000;
const ACCESS_MODEL_FAST_TIMEOUT_MS = 1600;
const ACCESS_MODEL_BACKGROUND_TIMEOUT_MS = 6500;
...
function getAccessCacheKey(uid) {
...
function normalizeProfileForState(profile) {
  if (!profile) return null;

  const trialEndsAtMs = valueToMillis(profile.trialEndsAt) || Number(profile.trialEndsAtMs) || 0;
  const accessUntilMs = valueToMillis(profile.accessUntil) || Number(profile.accessUntilMs) || 0;
...
function serializeClaimsForCache(claims = {}) {
  return {
    adminPanel: claims.adminPanel === true,
    fullAccess: claims.fullAccess === true
  };
}

function serializeProfileForCache(profile) {
  const normalized = normalizeProfileForState(profile);
  if (!normalized) return null;

  return {
    id: normalized.id || null,
    email: normalized.email || null,
    role: normalized.role || "trial",
    fullAccess: normalized.fullAccess === true,
    locationQuota: normalized.locationQuota || TRIAL_LOCATION_QUOTA,
    mapQuota: normalized.mapQuota || TRIAL_MAP_QUOTA,
    trialEndsAtMs: normalized.trialEndsAtMs || 0,
    accessUntilMs: normalized.accessUntilMs || 0
  };
}

function readCachedAccessModel(uid) {
  if (!uid) return null;

  try {
    const raw = safeLocalStorageGet(getAccessCacheKey(uid));
    if (!raw) return null;

    const cached = JSON.parse(raw);
    const cachedAt = Number(cached?.cachedAt) || 0;
    if (!cachedAt || Date.now() - cachedAt > ACCESS_CACHE_MAX_AGE_MS) return null;

    return {
      claims: cached.claims || {},
      profile: cached.profile ? { ...cached.profile, __cached: true } : null
    };
  } catch (error) {
    console.warn("Erişim önbelleği okunamadı:", error);
    return null;
  }
}

function writeCachedAccessModel(uid, claims, profile) {
  if (!uid || !profile || profile.__sessionFallback) return;

  const payload = {
    cachedAt: Date.now(),
    claims: serializeClaimsForCache(claims),
    profile: serializeProfileForCache(profile)
  };

  safeLocalStorageSet(getAccessCacheKey(uid), JSON.stringify(payload));
}

function buildSessionFallbackProfile(user) {
  return {
    id: user?.uid || null,
    ...
    trialEndsAtMs: Date.now() + TRIAL_DAYS * 24 * 60 * 60 * 1000,
    accessUntilMs: 0,
...
function openNoteEditor(target) {
  ...
  document.documentElement.classList.add("routee-note-editor-open");
  document.body?.classList.add("routee-note-editor-open");
...
function closeNoteEditor() {
  ...
  document.documentElement.classList.remove("routee-note-editor-open");
  document.body?.classList.remove("routee-note-editor-open");
...
function getTrialEndsAtMs() {
  return valueToMillis(state.profile?.trialEndsAt) || Number(state.profile?.trialEndsAtMs) || 0;
}

function getAccessUntilMs() {
  return valueToMillis(state.profile?.accessUntil) || Number(state.profile?.accessUntilMs) || 0;
}
...
function applyAccessModel(claims = {}, profile = null) {
  state.claims = claims || {};
  state.profile = normalizeProfileForState(profile);
  ...
  state.mapQuota = state.profile?.mapQuota || TRIAL_MAP_QUOTA;
  state.accessActive = hasActiveAccess();
}

async function loadAccessModel(user, options = {}) {
  const {
    timeoutMs = ACCESS_MODEL_FAST_TIMEOUT_MS,
    allowSessionFallback = true,
    useCache = true
  } = options;

  const cached = useCache ? readCachedAccessModel(user.uid) : null;
  if (cached?.profile || cached?.claims) {
    applyAccessModel(cached.claims || {}, cached.profile || state.profile);
  }

  const fallbackClaims = cached?.claims || state.claims || {};
  const fallbackProfile =
    cached?.profile || state.profile || (allowSessionFallback ? buildSessionFallbackProfile(user) : null);

  const claimsTask = getUserClaims(user, {
    timeoutMs,
    fallbackClaims,
    suppressErrors: true
  });

  const profileTask = withTimeoutFallback(
    getUserProfile(user.uid),
    timeoutMs,
    fallbackProfile
  ).catch((error) => {
    console.warn("Abonelik profili hızlı yüklenemedi:", error);
    return fallbackProfile;
  });

  const [claims, profile] = await Promise.all([claimsTask, profileTask]);
  const resolvedProfile = profile || fallbackProfile;

  applyAccessModel(claims || fallbackClaims, resolvedProfile);

  if (resolvedProfile && !resolvedProfile.__cached && !resolvedProfile.__sessionFallback) {
    writeCachedAccessModel(user.uid, claims || fallbackClaims, resolvedProfile);
  }
}

async function refreshAccessModelFromNetwork(user, options = {}) {
  if (!user) return;

  try {
    const [claims, profile] = await Promise.all([
      getUserClaims(user, {
        timeoutMs: ACCESS_MODEL_BACKGROUND_TIMEOUT_MS,
        fallbackClaims: state.claims || {},
        suppressErrors: true
      }),
      getUserProfile(user.uid)
    ]);
    ...
    applyAccessModel(claims || state.claims || {}, profile);
    writeCachedAccessModel(user.uid, claims || state.claims || {}, profile);
    ...
    console.warn("Erişim bilgisi arka planda güncellenemedi:", error);
  }
}
...
function initAuthWatcher() {
  watchAuth(async (user) => {
    ...
    await closeAppStartupSplash(state.appStartupSplash);
    state.appStartupSplash = null;
    elements.authStatus.textContent = `Aktif kullanıcı: ${user.email} · Erişim bilgisi kontrol ediliyor...`;

    const ensureProfileTask = ensureUserProfile(user.uid, user.email).catch((error) => {
      console.warn("Kullanıcı profili hızlı doğrulanamadı:", error);
    });
    ...
    try {
      ...
      console.warn("Erişim modeli hızlı başlatılamadı:", error);
      applyAccessModel(state.claims || {}, state.profile || buildSessionFallbackProfile(user));
      elements.authStatus.textContent = `Aktif kullanıcı: ${user.email} · Erişim bilgisi arka planda güncelleniyor...`;
    }
    ...
    ensureProfileTask.then(() => {
      refreshAccessModelFromNetwork(user, { reloadMaps: true });
    });
  });
}
