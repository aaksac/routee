...
import { locateAndShowUser, formatMeters } from "./location.js";
...
const state = {
  ...
  savedMapsLoadPromise: null,
  savedMapsLastLoadAt: 0,
  lastScrollY: 0,
...
const elements = {
  ...
  locationProgressNotice: document.getElementById("locationProgressNotice"),
  locationProgressNoticeText: document.getElementById("locationProgressNoticeText"),
...
const SAVED_MAPS_RETRY_COUNT = 1;
const SAVED_MAPS_RETRY_DELAY_MS = 700;
const SAVED_MAPS_REFRESH_STALE_MS = 20000;
const SAVED_MAPS_LOAD_TIMEOUT_MS = 12000;
...
async function refreshMapList(options = {}) {
  if (!state.currentUser) return;
  await loadUserMaps(state.currentUser.uid, isPremiumAccessActive(), options);
}
...
async function handleMapListClick(event) {
  const reloadBtn = event.target.closest("[data-action='reload-maps']");
  if (reloadBtn) {
    await refreshMapList({ showLoading: true, force: true });
    return false;
  }
  ...
  elements.authStatus.textContent = "Kayıtlı harita Firebase'den alınıyor...";
  ...
  try {
    ...
    if (!mapData) {
      await refreshMapList({ showLoading: true, force: true });
      elements.authStatus.textContent =
        "Harita Firebase'de bulunamadı ya da erişim yetkisi değişti. Liste yenilendi.";
      return false;
    }
    ...
    const safeStartPoint =
      startPoint && Number.isFinite(startPoint.lat) && Number.isFinite(startPoint.lng)
        ? startPoint
        : null;
    const safeEndPoint =
      endPoint && Number.isFinite(endPoint.lat) && Number.isFinite(endPoint.lng)
        ? endPoint
        : null;

    const applied = applyImportedData(safeStartPoint, points, safeEndPoint);
    if (!applied) {
      elements.authStatus.textContent =
        "Harita verisi alındı fakat ekrana uygulanamadı. Konum limiti veya veri içeriği kontrol edilmeli.";
      return false;
    }

    state.selectedMapId = mapData.id;
    elements.mapName.value = mapData.name || "";
    focusMapToPoints(safeStartPoint, points, safeEndPoint);
...
function showLocationProgressNotice(message = "Konumunuz belirleniyor...") {
  if (elements.locationProgressNoticeText) {
    elements.locationProgressNoticeText.textContent = message;
  }

  elements.locationProgressNotice?.classList.remove("hidden");
  elements.btnCurrentLocation?.classList.add("is-busy");
  elements.btnCurrentLocation?.setAttribute("aria-busy", "true");
  elements.btnCurrentLocation?.setAttribute("disabled", "disabled");
}

function hideLocationProgressNotice() {
  elements.locationProgressNotice?.classList.add("hidden");
  elements.btnCurrentLocation?.classList.remove("is-busy");
  elements.btnCurrentLocation?.removeAttribute("aria-busy");
  elements.btnCurrentLocation?.removeAttribute("disabled");
}

async function handleCurrentLocationClick() {
  const FORM_FILL_ACCURACY_LIMIT_METERS = 250;

  if (state.currentLocationBusy) return;
  state.currentLocationBusy = true;

  try {
    showLocationProgressNotice("Konumunuz belirleniyor...");
    elements.authStatus.textContent = "Konumunuz belirleniyor; GPS hassasiyeti bekleniyor...";

    const coords = await locateAndShowUser();
    const lat = Number(coords?.lat);
    const lng = Number(coords?.lng);
    const accuracy = Number(coords?.accuracy);
    const hasGoodAccuracy = !Number.isFinite(accuracy) || accuracy <= FORM_FILL_ACCURACY_LIMIT_METERS;

    if (Number.isFinite(lat) && Number.isFinite(lng) && hasGoodAccuracy) {
    ...
    if (Number.isFinite(accuracy)) {
      elements.authStatus.textContent = hasGoodAccuracy
        ? `Mevcut konum haritada gösterildi. Doğruluk: yaklaşık ${formatMeters(accuracy)}.`
        : `Konum bulundu fakat hassas değil: yaklaşık ${formatMeters(accuracy)}. Yanlış kayıt oluşmaması için forma otomatik yazılmadı. Android ayarlarında Kesin konumu açın.`;
    } else {
    ...
    hideLocationProgressNotice();
    state.currentLocationBusy = false;
  }
}
...
function loadMapListLoadingMessage() {
  elements.mapList.innerHTML = `<div class="map-list-item"><strong>Kayıtlı haritalar yükleniyor...</strong></div>`;
}

function loadMapListErrorMessage(errorMessage = "") {
  const detail = errorMessage
    ? escapeHtml(errorMessage)
    : "Bağlantı hazır olmayabilir. Tekrar deneyin.";

  elements.mapList.innerHTML = `
    <div class="map-list-item">
      <strong>Haritalar yüklenemedi</strong>
      <span>${detail}</span>
    </div>
    <button class="tiny-btn" type="button" data-action="reload-maps">Tekrar yükle</button>
  `;
}

async function loadUserMaps(uid, fullAccess, options = {}) {
  if (!uid) {
    elements.mapList.innerHTML = `<div class="map-list-item"><strong>Haritaları görmek için giriş yapmalısınız.</strong></div>`;
    return;
  }
  ...
  const maxRetries = Number.isFinite(Number(options.retries))
    ...
    : SAVED_MAPS_RETRY_COUNT;

  if (state.savedMapsLoading && state.savedMapsLoadPromise) {
    if (showLoading && !elements.mapList?.children?.length) {
      loadMapListLoadingMessage();
    }

    if (force) {
      // Kullanıcı elle yenileme yaptıysa eski/askıda kalmış yükleme isteğini bekleme.
      // Aksi halde panel "Kayıtlı haritalar yükleniyor..." durumunda kilitlenebilir.
      state.savedMapsLoading = false;
      state.savedMapsLoadPromise = null;
    } else {
      try {
        await withTimeout(
          state.savedMapsLoadPromise,
          SAVED_MAPS_LOAD_TIMEOUT_MS,
          "Önceki harita listesi yükleme isteği zaman aşımına uğradı."
        );
      } catch (error) {
        state.savedMapsLoading = false;
        state.savedMapsLoadPromise = null;
  ...
  const recentlyLoaded = Date.now() - state.savedMapsLastLoadAt < SAVED_MAPS_REFRESH_STALE_MS;
  if (!force && recentlyLoaded && elements.mapList?.children?.length) return;
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        try {
          const maps = await withTimeout(
            getMaps(uid, { fullAccess }),
            SAVED_MAPS_LOAD_TIMEOUT_MS,
            "Harita listesi zaman aşımına uğradı. İnternet bağlantınızı kontrol edip Tekrar yükle düğmesine basın."
          );
          state.savedMapsLastLoadAt = Date.now();
            ...
            .map(
              (map) => `
                <div class="map-list-row">
                  <button class="map-list-item ${state.selectedMapId === map.id ? "active" : ""}" type="button" data-map-id="${escapeHtml(map.id)}">
                    <strong>${escapeHtml(map.name || "İsimsiz Harita")}</strong>
                    <span>Toplam mesafe: ${formatKm(map.totalDistance || 0)}</span>
                  </button>
                  <button class="tiny-btn danger-outline" type="button" data-action="delete-map" data-map-id="${escapeHtml(map.id)}">
                    Sil
                  </button>
                </div>
              `
          ...
          await wait(SAVED_MAPS_RETRY_DELAY_MS * (attempt + 1));
        }
      }

      throw lastError || new Error("Harita listesi alınamadı.");
    } catch (error) {
      const message = error?.message || "Bilinmeyen hata";
      loadMapListErrorMessage(message);
      elements.authStatus.textContent = `Haritalar yüklenemedi: ${message}`;
    }
  })();

  state.savedMapsLoading = true;
  state.savedMapsLoadPromise = loadTask;

  try {
    await loadTask;
  } finally {
    state.savedMapsLoading = false;
    if (state.savedMapsLoadPromise === loadTask) {
      state.savedMapsLoadPromise = null;
    }
  }
}
...
async function openSavedMapsOverlay() {
  closeFloatingPanels();
  closeMapMenu();
  closeLocationActionMenu();
  ...
  if (!state.currentUser) {
    elements.mapList.innerHTML = `<div class="map-list-item"><strong>Oturum hazırlanıyor...</strong><span>Kısa süre sonra tekrar deneyin.</span></div>`;
    return;
  }
...
function initMobileDeadZoneTapGuards() {
  const guardedAreas = [
    elements.topbar,
    document.querySelector(".trip-panel-header")
  ].filter(Boolean);

  if (!guardedAreas.length) return;
  ...
  const isDeadZoneTap = (event, area) => {
    if (!isMobileTapSurface()) return false;
    ...
    try {
      if (user) {
        ...
        const loadMapsTask = async () => {
          try {
            await loadUserMaps(user.uid, isPremiumAccessActive(), { showLoading: true, force: true });
          } catch (error) {
