...
function getAccessStatusText() {
  if (isPremiumAccessActive()) {
    const accessUntil = getAccessUntilMs();
    if (accessUntil) {
      return `Tam erişim · Bitiş: ${new Date(accessUntil).toLocaleDateString("tr-TR")}`;
    }
    return "Tam erişim";
  }

  if (isTrialActive()) {
    return `Deneme hesabı · En fazla ${state.locationQuota} konum · Tek kayıtlı harita`;
  }
    ...
    ? `
      <div class="trip-item start">
        ...
        <div style="display:flex; gap:8px; align-items:center;">
          <button class="tiny-btn" type="button">Yol Tarifi Al</button>
